...
{
    public class TodoListEndpointGroup : IEndpointGroup
    {
        ...
        public static async Task<Ok<IReadOnlyCollection<TodoListDto>>> GetTodoLists(ISender sender)
        {
            var vm = await sender.Send(new GetTodoListsQuery());
